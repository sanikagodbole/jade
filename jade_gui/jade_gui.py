...
def render_base_folder_input() -> Optional[Path]:
    ...
    if base_path:
        ...
        if base_path.exists():
            ...
            st.warning(f"⚠️ Path not found: {base_path}")
            return None
    return None
...
def render_directory_viewer(base_path: Path):
    ...
    with col1:
        if st.button("📂 Expand", use_container_width=True, key="expand_btn"):
            st.session_state.expand_all = True
    with col2:
        if st.button("📁 Collapse", use_container_width=True, key="collapse_btn"):
    ...
    if base_path.exists():
        ...
        st.warning("⚠️ Base folder not found. Please check the path.")
...
def render_actions_panel():
    ...
    if st.button("📦 Publish", use_container_width=True, key="action_publish_asset"):
